...
create table if not exists delivery_attempts (
  ...
  -- The customer's own answer to the notice we sent about this attempt, so the
...
alter table delivery_attempts add column if not exists customer_replied_at timestamptz;
alter table delivery_attempts add column if not exists customer_reply text not null default '';
...
-- Serves both the per-order message audit and the customer cooldown lookup that
